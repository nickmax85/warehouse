 ...
 */
public class ScanOverviewController implements Initializable {
	...
	private void handleGetArticles() {
		...
		articles.addAll(Service.getInstance().getArticlesFromToDate(startDatumConvert, endDatumConvert));
		...
		if (event.getEventType() == KeyEvent.KEY_PRESSED)
			...
			if (event.getCode() == KeyCode.ENTER) {
				...
				if (!nrTextField.getText().isEmpty()) {
					...
					Service.getInstance().insertArticle(article);
	...
	public void exportToCSV() throws Exception {
		...
		try {
			...
			String dateString = df.format(endDatumConvert);

			fileChooser.setSelectedFile(new File(dateString + "-" + timeString + ".csv"));
			...
			for (Article article : Service.getInstance().getArticlesFromToDate(startDatumConvert, endDatumConvert)) {
